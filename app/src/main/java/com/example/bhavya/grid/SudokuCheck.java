...
public class SudokuCheck {
    private static SudokuCheck instance;

    private SudokuCheck(){
    }
    public static SudokuCheck getInstance(){
        if(instance == null){
            instance=new SudokuCheck();
        }
        return instance;
    }

    public boolean checkGame(int[][] Sudoku){
    ...
    private boolean checkRegions(int[][] Sudoku) {
        for(int xRegion = 0; xRegion < 3; xRegion++) {
            for (int yRegion = 0; yRegion < 9; yRegion++) {
